  ...
  return (
    <div ref={ref} className="w-full h-24 md:h-32 flex items-center justify-center -mt-8 mb-4 cursor-grab">
      ...
      >
        ...
        />
        <svg viewBox="0 0 100 80" className="absolute inset-0 w-full h-full drop-shadow-xl" style={{ transform: 'translateZ(25px)' }}>
          <defs>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="w-full max-w-md relative z-10">
        <Card className="bg-white/40 dark:bg-slate-800/40 backdrop-blur-lg border border-slate-200/80 dark:border-slate-700/80 rounded-2xl shadow-lg">
          <CardHeader className="text-center p-6 md:p-8">
            ...
            <CardTitle className="text-2xl md:text-3xl font-bold font-sora text-slate-900 dark:text-white">Welcome Back</CardTitle>
          ...
          <CardContent className="p-6 md:p-8 pt-0 space-y-4">
            ...
            <div className="relative">
              ...
              <div className="relative flex justify-center text-xs uppercase"><span className="bg-white/50 dark:bg-slate-800/50 px-2 text-slate-500">Or continue with</span></div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input id="email" type="email" autoComplete="email" value={email} onChange={(e) => setEmail(e.target.value)} className={`pl-10 h-12 bg-white/50 dark:bg-slate-900/50 ${errors.email ? 'border-red-500' : ''}`} disabled={loadingState !== 'idle'}/>
                </div>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input id="password" type={showPassword ? 'text' : 'password'} autoComplete="current-password" value={password} onChange={(e) => setPassword(e.target.value)} className={`pl-10 pr-10 h-12 bg-white/50 dark:bg-slate-900/50 ${errors.password ? 'border-red-500' : ''}`} disabled={loadingState !== 'idle'}/>
                  <button type="button" className="absolute right-3 top-1/2 -translate-y-1/2 p-2" onClick={() => setShowPassword(!showPassword)} aria-label={showPassword ? "Hide password" : "Show password"}>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-2">
                  <input type="checkbox" id="remember-me" checked={rememberMe} onChange={(e) => setRememberMe(e.target.checked)} className="peer h-5 w-5 shrink-0 rounded-sm border border-slate-300 dark:border-slate-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-500 checked:bg-sky-600 checked:border-sky-600"/>
                  <Label htmlFor="remember-me" className="text-sm font-normal text-slate-600 dark:text-slate-400 cursor-pointer">Remember me</Label>
                </div>
                <Link to="/forgot-password" className="text-sm font-semibold text-sky-600 dark:text-sky-400 hover:underline p-1">
                  Forgot password?
                </Link>
              </div>
          ...
          <CardFooter className="p-6 md:p-8 justify-center bg-black/5 dark:bg-black/10 border-t border-slate-200/50 dark:border-slate-700/50">
            <p className="text-sm text-slate-600 dark:text-slate-400">Don't have an account? <Link to="/register" className="font-semibold text-sky-600 dark:text-sky-400 hover:underline p-1">Sign up</Link></p>
          </CardFooter>
        </Card>
      </motion.div>
    </div>
