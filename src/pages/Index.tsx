...
import {
  ...
  History as HistoryIcon, Loader2, Star, Settings, CheckCircle,
...
export default function Index() {
  ...
  const handleMakeAdmin = async () => {
    if (!user || user.email !== 'js.clarityocr@gmail.com') {
      toast({ title: "Action restricted to the owner.", variant: "destructive" });
      return;
    }
    try {
      const functions = getFunctions();
      const grantAdminRole = httpsCallable(functions, 'grantAdminRole');
      toast({ title: "Requesting Admin Role...", description: "Please wait." });
      const result = await grantAdminRole({ email: 'js.clarityocr@gmail.com' });
      
      console.log("Function result:", result.data);
      toast({
        title: "Admin Role Granted! ✅",
        description: "CRITICAL: Please LOG OUT and LOG BACK IN to apply the changes.",
        duration: 7000,
      });
    } catch (error: any) {
      console.error("Error granting admin role:", error);
      toast({ title: "Admin Grant Failed ❌", description: error.message, variant: "destructive" });
    }
  };
  ...
  const UserNav = () => (
    <>
      ...
      {user?.admin && <Button variant="secondary" onClick={() => navigate('/admin')}>Admin Panel</Button>}
      <div className="w-px h-6 bg-slate-200 dark:bg-slate-700 mx-2"></div>
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button variant="ghost" className="relative h-10 w-10 rounded-full"><User className="h-6 w-6 text-slate-600 dark:text-slate-300" /></Button>
        </DropdownMenuTrigger>
      ...
      <nav className="sticky top-0 z-50 bg-white/80 dark:bg-slate-900/80 backdrop-blur-lg border-b border-slate-200 dark:border-slate-800 transition-shadow hover:shadow-md">
        <div className="container mx-auto px-4 py-3">
          <div className="flex items-center justify-between">
            ...
            <div className="hidden md:flex items-center gap-2">{user && <UserNav />}</div>
          </div>
          {mobileMenuOpen && (<motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} className="md:hidden mt-4 pb-4 space-y-2 border-t border-slate-200 dark:border-slate-700 overflow-hidden">
            ...
            {user?.admin && <Button variant="ghost" className="w-full justify-start text-base py-6" onClick={() => { navigate('/admin'); closeMobileMenu(); }}>Admin Panel</Button>}
            <Button variant="ghost" className="w-full justify-start text-base py-6" onClick={() => { navigate('/profile'); closeMobileMenu(); }}>Profile & Settings</Button>
          </motion.div>)}
        </div>
      </nav>
