...
import {
  ...
  ArrowDown // UPDATED: Changed ArrowRight to ArrowDown
} from 'lucide-react';
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-slate-200">
      ...
      <main>
          ...
          <div className="container mx-auto px-4">
            ...
            <motion.div variants={containerVariants} initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.3 }} className="grid grid-cols-1 md:grid-cols-3 gap-8 text-center">
              <motion.div variants={itemVariants}>
                <div className="flex justify-center items-center mb-4 w-16 h-16 mx-auto bg-indigo-100 dark:bg-indigo-900/50 rounded-full border-8 border-indigo-50 dark:border-indigo-900/30">
                ...
                <p className="text-slate-600 dark:text-slate-400">Securely upload your file or paste text directly into the analyzer.</p>
              </motion.div>
              <motion.div variants={itemVariants}>
                <div className="flex justify-center items-center mb-4 w-16 h-16 mx-auto bg-purple-100 dark:bg-purple-900/50 rounded-full border-8 border-purple-50 dark:border-purple-900/30">
                  <ScanLine className="w-8 h-8 text-purple-600 dark:text-purple-400" />
                </div>
                <h3 className="text-xl font-semibold mb-2">2. AI Analysis</h3>
                <p className="text-slate-600 dark:text-slate-400">Our AI engine performs OCR and extracts actionable tasks in seconds.</p>
              </motion.div>
              <motion.div variants={itemVariants}>
                <div className="flex justify-center items-center mb-4 w-16 h-16 mx-auto bg-green-100 dark:bg-green-900/50 rounded-full border-8 border-green-50 dark:border-green-900/30">
                  <ListChecks className="w-8 h-8 text-green-600 dark:text-green-400" />
                </div>
                <h3 className="text-xl font-semibold mb-2">3. Get Checklist</h3>
                <p className="text-slate-600 dark:text-slate-400">Receive an organized, editable checklist with priorities and deadlines.</p>
              </motion.div>
            </motion.div>
          </div>
        </section>
        ...
        <section id="features" className="py-20 bg-slate-100 dark:bg-slate-900/50">
          <div className="container mx-auto px-4">
            <div className="text-center mb-12">
              <h2 className="text-3xl md:text-4xl font-bold text-slate-800 dark:text-slate-200">Powerful Features, Simple Interface</h2>
              <p className="mt-4 text-lg text-slate-600 dark:text-slate-400 max-w-3xl mx-auto">Everything you need to streamline your document workflow and boost productivity.</p>
            </div>
            <motion.div variants={containerVariants} initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.1 }} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {FEATURES_DATA.map((feature) => (
                <motion.div variants={itemVariants} key={feature.title}>
                  <Card className="bg-white dark:bg-slate-800 h-full hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
                    <CardHeader>
                      <div className="bg-gradient-to-br from-indigo-500 to-purple-600 p-3 rounded-lg w-fit mb-4 shadow-lg shadow-indigo-500/20">
        ...
        <section id="testimonials" className="py-20">
          <div className="container mx-auto px-4">
            <div className="text-center mb-12">
              <h2 className="text-3xl md:text-4xl font-bold text-slate-800 dark:text-slate-200">Trusted by Professionals Worldwide</h2>
              <p className="mt-4 text-lg text-slate-600 dark:text-slate-400">See what our users are saying about Clarity OCR.</p>
            </div>
            <motion.div variants={containerVariants} initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.3 }} className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    ...
                    <CardContent className="pt-6 flex flex-col h-full">
                      <div className="flex-grow">
                        <div className="flex mb-3">
                          {[...Array(5)].map((_, i) => <Star key={i} className={`w-5 h-5 ${i < testimonial.rating ? 'text-yellow-400 fill-current' : 'text-gray-300'}`} />)}
                        </div>
                        <p className="text-slate-600 dark:text-slate-300 italic mb-6">"{testimonial.content}"</p>
                      </div>
                      <div className="flex items-center gap-4 border-t border-slate-200 dark:border-slate-700 pt-4 mt-auto">
                        <div className="w-12 h-12 rounded-full bg-slate-200 dark:bg-slate-700 flex items-center justify-center font-bold text-indigo-600 dark:text-indigo-400 text-lg">
                          {testimonial.avatar}
                        </div>
                        <div>
        ...
        <section id="faq" className="py-20 bg-slate-100 dark:bg-slate-900/50">
          <div className="container mx-auto px-4 max-w-4xl">
            <div className="text-center mb-12">
              <h2 className="text-3xl md:text-4xl font-bold text-slate-800 dark:text-slate-200">Frequently Asked Questions</h2>
            </div>
            <Accordion type="single" collapsible className="w-full">
              {FAQ_ITEMS.map((faq) => (
                <AccordionItem value={faq.question} key={faq.question}>
                  <AccordionTrigger className="text-lg text-left">{faq.question}</AccordionTrigger>
                  <AccordionContent className="text-base text-slate-600 dark:text-slate-400">{faq.answer}</AccordionContent>
                </AccordionItem>
              ))}
            </Accordion>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
            <div className="md:col-span-2">
              <Link to="/" className="flex items-center gap-3 mb-4">
                ...
                <h1 className="text-xl font-bold text-slate-100">Clarity OCR</h1>
              </Link>
              <p className="max-w-md">Transforming unstructured documents into actionable insights with the power of AI.</p>
            </div>
            <div>
