...
// Define the path to redirect to after successful registration for easy configuration
const HOME_PATH = '/';
...
export default function Register() {
  ...
  // This useEffect handles auth state on mount (redirects, existing sessions)
  useEffect(() => {
    getRedirectResult(auth)
      .then((result) => {
        if (result?.user) {
          toast({ title: 'Sign-In Successful', description: `Welcome, ${result.user.displayName || 'User'}!` });
          navigate(HOME_PATH);
        } else {
        ...
        <Card className="shadow-xl bg-white rounded-2xl">
          ...
          <CardContent className="p-8">
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="relative text-center"><span className="text-xs uppercase text-gray-400 bg-white px-2 relative z-10">Or sign up with</span><hr className="absolute top-1/2 w-full -z-0"/></div>
